 ...
 * @param {string} contentType - 内容类型 ('subtitle' | 'comments')
...
## ${contentType === 'subtitle' ? '字幕内容' : '热门评论'}
...
/**
 * B站视频典藏主函数
 * @param {string} url - B站视频 URL（bilibili.com/video/BVxxx 或 b23.tv/xxx）
 * @param {ClipperConfig} config - 配置对象
 * @returns {Promise<ClipperResult>} 执行结果
 */
async function bilibiliClip(url, config) {
    console.log('\n🎬 === 开始B站典藏 ===');
    console.log(`URL: ${url}`);
    ...
    try {
        // 3. 获取视频元数据
        const metadata = await getBilibiliMetadata(bvid);
        console.log(`✅ 元数据获取完成：${metadata.title} - ${metadata.author}`);

        // 4. 尝试获取字幕
        const subtitleResult = await downloadBilibiliSubtitle(bvid, outputDir);

        // 5. 构建临时 Markdown（字幕或评论）
        let contentPath;
        let contentType;
        if (subtitleResult.exists) {
            contentPath = subtitleResult.subtitlePath;
            contentType = 'subtitle';
            console.log('✅ 使用字幕内容生成总结');
        } else {
            // 无字幕时获取评论
            const commentsResult = await downloadBilibiliComments(bvid, outputDir);
