...
/**
 * 知乎内容典藏主函数
 * @param {string} url - 知乎 URL（文章/回答/问题）
 * @param {ClipperConfig} config - 配置对象
 * @returns {Promise<ClipperResult>} 执行结果
 */
async function zhihuClip(url, config) {
    console.log('\n📘 === 开始知乎典藏 ===');
    console.log(`URL: ${url}`);
    ...
    try {
        // 2. 使用 opencli 下载知乎内容 → { mdPath, title, originalUrl, source: 'zhihu' }
        const downloadResult = await download('zhihu', url, outputDir);
